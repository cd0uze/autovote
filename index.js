...
async function autovote(i) {
    await browser.newPage().then(async page => {
        ...
        await page.goto(Config.sites[i].url, {waitUntil: Config.sites[i].index !== 3 ? "domcontentloaded" : "networkidle2", timeout: 0});
    } else {
        await page.goto(Config.sites[i].url, {waitUntil: Config.sites[i].index !== 3 ? "domcontentloaded" : "networkidle0", timeout: 0});
    }
